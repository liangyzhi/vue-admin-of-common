            ...
            <a-row :gutter="20">
                ...
                <a-col :span='16'>
                    <a-form layout="horizontal" :form="form">
                        ...
                        <a-form-item label="路径" :label-col="{ span: 7 }" :wrapper-col="{ span: 12 }" >
                            <a-input placeholder="请输入" v-decorator="['location', {rules: [{ required: true, message: '请输入路径' }]}]" :disabled="formDisabled"/>
                        </a-form-item>
                        ...
                        <a-form-item label="类型" :label-col="{ span: 7 }" :wrapper-col="{ span: 12 }" >
                            <a-radio-group v-decorator="['type',{initialValue:'MENU',rules: [{ required: true, message: '请输入' }]}]" :disabled="formDisabled">
                                <a-radio value="MENU">菜单</a-radio>
                                <a-radio value="BUTTON">按钮</a-radio>
                            </a-radio-group>
                        </a-form-item>
                        <a-form-item label="图标" :label-col="{ span: 7 }" :wrapper-col="{ span: 12 }" >
                            <a-input placeholder="请输入" v-decorator="['icon',{rules: [{ required: false, message: '请输入图标' }]}]" :disabled="formDisabled"/>
                        </a-form-item>
...
export default {
	...
	methods: {
		...
		getTree () {
			this.loading = true
			this.$axios.get('/fline/permission/menu/tree').then((data) => {
		...
		save () {
			if (this.okBtn !== 0) {
				var url = '/fline/permission/update'
				this.saveAction(url, '修改')
			} else {
				var url = '/fline/permission/create'
		...
		edit () {
			...
			if (this.selectedKeys.length == 0) {
				this.$message.success('请选择要修改的菜单！')
				this.formDisabled = true
			} else {
				this.formDisabled = false
				setTimeout(() => {
				this.form.setFieldsValue({
					...
					location: this.nodeData.location,
					code: this.nodeData.code,
					type: this.nodeData.type,
					icon: this.nodeData.icon,
					ordinal: this.nodeData.ordinal,
		...
		// 删除树节点
		onDelete () {
			if (this.nodeData.children !== null && this.nodeData.children.length !== 0) {
			...
			if (this.selectedKeys !== '') {
				...
				this.$confirm({
					...
					cancelText: '取消',
					onOk () {
						that.$axios.delete(`/fline/permission/remove?id=${that.nodeData.id}&version=${that.nodeData.version}`).then(data => {
							if (data.data.code === 200) {
								that.$message.success(data.data.msg)
							}
							that.getTree()
