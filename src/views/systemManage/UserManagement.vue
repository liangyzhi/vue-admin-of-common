    ...
    export default{
        ...
        methods:{
            ...
            getTableData(){
                ...
                this.$axios.get('/fline/user/page',{params}).then((data) => {
                    this.loading = false;
            ...
            usernameValidate (rule, value, callback) {
                if(this.okBtn!==1){
                    this.$axios.get(`/fline/user/username/exists/${value}`).then(res=>{
            ...
            //删除用户名
            onDelete (datas) {
                this.$axios.delete('/fline/user/remove',{data:{"id":datas.id,version:datas.version}}).then(res=>{
                    if (res.data.code == 200) {
