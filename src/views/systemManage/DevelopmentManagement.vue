...
export default {
	...
	methods: {
		...
		save () {
			if (this.okBtn !== 0) {
				var url = '/fline/department/update'
				this.saveAction(url, '修改')
			} else {
				var url = '/fline/department/create'
		...
		onDelete () {
			...
			if (this.selectedKeys !== '') {
				...
				this.$confirm({
					...
					cancelText: '取消',
					onOk () {
						that.$axios.delete(`/fline/department/remove?id=${that.nodeData.id}&version=${that.nodeData.version}`).then(data => {
							if(data.data.code==200){
								that.getTree()
								that.formDisabled = true
							}
