    ...
    export default{
        ...
        methods:{
            ...
            getTableData(){
                ...
                this.$axios.get('/fline/user/page',{params:params}).then((data) => {
                    this.loading = false;
            ...
            usernameValidate (rule, value, callback) {
                if(this.okBtn!==1){
                    this.$axios.get(`/fline/user/username/exists/${value}`).then(data=>{
                        if (data.data.code === 200) {
            ...
            //删除用户名
            onDelete (datas) {
                this.$axios.delete('/fline/user/remove',{data:{"id":datas.id,version:datas.version}}).then(data=>{
                    if (data.data.code === 200) {
            ...
            handleOk(e) {
                if(this.okBtn==0){
                    var url='/fline/user/create';
                }else{
                    var url='/fline/user/update';
                }
                e.preventDefault();
            ...
            getUserRole(){
                if(JSON.stringify(this.tableSelectData)!=='{}'){
                    ...
                    this.rolesData=[];
                    this.$axios.get('/fline/role/all').then((data) => {
                        if(data.data.code==200){
            ...
            handleOkRole(e) {
                ...
                this.roleForm.validateFieldsAndScroll((err, values) => {
                    if (!err) {
                        this.confirmLoading = true;
                        this.$axios.post('/fline/user/assignRoles', this.$qs.stringify({
            ...
            getTree() {
                this.loading = true
                this.$axios.get('/fline/department/tree').then((data) => {
                    if (data.data.code === 200) {
