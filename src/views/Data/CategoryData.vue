<template>
    <div class="commonFormContainer">
            ...
            <a-row :gutter='30'>
                <a-col :span="6">
                    <a-form-item label="关键词">
                        <a-input
                        placeholder="请输入关键词!"
                          v-decorator="['keyword', { rules: [{ required: false, message: '请输入关键词!' }] }]"
                        />
                     </a-form-item>
                </a-col>
                <a-col :span="6">
                    <a-form-item label="名称">
                      <a-input
                      placeholder="请输入名称!"
                        v-decorator="['name', { rules: [{ required: false, message: '请输入名称!' }] }]"
                      />
                    </a-form-item>
                </a-col>
                <a-col :span="6">
                    <a-form-item label="编码">
                      <a-input
                      placeholder="请输入编码"
                        v-decorator="['code', { rules: [{ required: false, message: '请输入编码!' }] }]"
                      />
                    </a-form-item>
                </a-col>
                <a-col :span="6">
        ...
        <!-- 编辑 -->
        <category-edit :editVisible='editVisible' @editClose='editClose' @editSubmit='editSubmit' :editData='editData'>
        </category-edit>
        <!-- 关联单位 -->
        <category-relevance :relevanceVisible='relevanceVisible' @relevanceClose='relevanceClose' @relevanceSubmit='relevanceSubmit' :relevanceData='relevanceData'>
        </category-relevance>
        <!-- 增加 -->
        <category-add :addVisible='addVisible' @addClose='addClose' @addSubmit='addSubmit'>
        </category-add>
    </div>
</template>
...
<script>
    ...
    import CategoryRelevance from './category/CategoryRelevance.vue'
            ...
            getCategory() {
                this.form.validateFields((error, values) => {
                    ...
                    values.createDate = this.timeString
                     this.$axios.get('fline/dataCategory/findBy', {params:values})
                     .then(res => {
                         if (res.data.code == 200) {
